    ...
    <body>
        <div class="container">
            ...
            <div id="search">
                ...
                <input type="button" value="Delete" onclick="deleteUserChoosed()" 
                    class="btn btn-sm btn-danger btn-delete" style="margin-left: 41%; display: none;" />
            </div>
            <table class="table" id="myTable">
                <thead>
                    <tr>
                        <td><input type="checkbox" class="form-check-input all-checkbox" onclick="selectAllCheckbox()"></td>
                        <th>User Id
                ...
                <tbody id="table-body">
                    <c:forEach var="user" items="${userList}">
                        <tr>
                            <td><input type="checkbox" class="form-check-input checkbox-idlist" value="${user.userId}" ></td>
                            <td>${user.userId}</td>
                            <td>${user.name}</td>
        ...
        <script type="text/javascript">
            ...
            function selectAllCheckbox() {
                var selected = $('.checkbox-idlist');
                var arrSelect = Array.prototype.slice.call(selected);
				if($('.all-checkbox').checked){
					$('.btn-delete').css('display','inline');
                    for(var i = 0; i < arrSelect.length; i++){
                        arrSelect[i].checked = true;
                    }
	            }else{
                	$('.btn-delete').css('display','none');
                	for(var i = 0; i < arrSelect.length; i++){
                        arrSelect[i].checked = false;
                    }
                }
                
            }
        
            function deleteUserChoosed() {
            ...
            function sortName(typeSort, position){
                ...
                while (switching) {
                    ...
                    for (i = 1; i < (rows.length - 1); i++) {
                      shouldSwitch = false;
                      x = rows[i].getElementsByTagName("TD")[position]// position=vij trí cần sort;
